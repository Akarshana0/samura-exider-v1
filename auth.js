...
// NOTE: wrapped in try/catch — a Firestore write failure must NOT
// crash authentication. The user is already signed in at this point.
...
export async function loginWithEmail(email, password) {
  ...
  // setSessionToken is non-fatal (wrapped internally) — login always succeeds
  await setSessionToken(cred.user.uid);
  return cred.user;
}

// ─── Google Sign-In ─────────────────────────────────────────
// signInWithPopup is used for ALL devices now.
// signInWithRedirect is deprecated on modern browsers (Chrome 115+) due to
// third-party storage partitioning — it breaks silently on mobile.
