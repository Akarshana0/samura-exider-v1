...
import { registerWithEmail, loginWithEmail, loginWithGoogle, handleGoogleRedirect } from "../firebase/auth";
...
export default function AuthPage() {
  ...
  useEffect(() => {
    handleGoogleRedirect()
      .then((user) => { if (user) navigate("/portal", { replace: true }); })
      .catch((err) => {
        const msg = AUTH_ERRORS[err?.code] || "Google sign-in failed. Please try again.";
        setError(msg);
      });
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // Desktop: user returned immediately. Mobile: null (redirect in progress).
      if (user) navigate("/portal");
      // On mobile the page will reload automatically — no further action needed.
    } catch (err) {
      setError(AUTH_ERRORS[err.code] || "Google sign-in failed. Please try again.");
      setLoading(false);
    } finally {
      // Don't turn off loading on mobile redirect (page reloads anyway)
      if (!('ontouchstart' in window)) setLoading(false);
    }
  };
